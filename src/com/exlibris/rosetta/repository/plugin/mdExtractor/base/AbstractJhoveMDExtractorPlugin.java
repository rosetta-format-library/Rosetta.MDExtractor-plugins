...
public abstract class AbstractJhoveMDExtractorPlugin implements MDExtractorPlugin {
    ...
    protected static String extractJhoveValue(String[] fieldPath, Property parentProperty, int fieldPathCurrentSegment) {
...
//            String propString = ColectionPropertyToString(((Collection) object).toArray());
            String propString = ColectionPropertyToString(arr);
            return "[" + propString.substring(0, propString.length() - 2) + "]";
        }
    ...
    public String getAttributeByName(int position, String prefixAttributeName, String suffixAttributeName) {
        if (repinfo.getProperty() == null) {
            return null;
        }

        try {
            String[] prefixJhoveFieldPath = prefixAttributeName.split("\\.");
            Object currentProperty = repinfo.getProperty().get(prefixJhoveFieldPath[0]);

            for (int i = 0; i < prefixJhoveFieldPath.length; i++) {
                if (currentProperty instanceof Property){
                    currentProperty=((Property)currentProperty).getValue();
                }else{
                    return null;
                }
            }

            if (!(currentProperty instanceof Property) || !(((Property)currentProperty).getArity().equals(PropertyArity.LIST))){
                return null;
            }

            List<Property> list=(ArrayList<Property>)((Property)currentProperty).getValue();
            if (list.size()<=position){
                return null;
            }

            String[] jhoveFieldPath = suffixAttributeName.split("\\.");
            if (suffixAttributeName.endsWith(".")) {
                jhoveFieldPath[jhoveFieldPath.length - 1] = jhoveFieldPath[jhoveFieldPath.length - 1] + ".";
            }

            String propertyValue = extractJhoveValue(jhoveFieldPath, list.get(position), 0);
    ...
    private static String ColectionPropertyToString(Object object) {
        ...
        }// object is a property of Map
        else if (object instanceof Property && ((Property) object).getArity().equals(PropertyArity.MAP)) {
            List<String> list = new ArrayList<>();
            Property prop = (Property) object;
            Map values = (LinkedHashMap) prop.getValue();
            values.forEach((k, v) -> {
                list.add(k + "=" + v);
            });
